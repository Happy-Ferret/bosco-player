...
export default class GResource
    constructor: (config) ->
        @resName = config.res_name
        @resPrefix = config.res_prefix
        @files = []

    setName: (name) -> @resName = name

    setPrefix: (prefix) -> @resPrefix = prefix
    ...
    build: () ->
        ...
        xml += "\t</gresource>\n</gresources>\n"
        xmlfile = @base.resolve_relative_path(@resName + ".xml")
    ...
    compile: (next = () => {}) ->
        try 
            [ ok, pid ] = GLib.spawn_async(@base.get_path(),
                                [ "glib-compile-resources", @resName + ".xml" ],
                                GLib.get_environ(),
            ...
            GLib.child_watch_add GLib.PRIORITY_DEFAULT, pid, () => 
                ...
                try 
                    notification = new Notify.Notification(
                        ...
                        body: @resName + " generated at " + @base.get_path()

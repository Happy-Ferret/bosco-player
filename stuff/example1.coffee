...
ListBoxRowWithData = Lang.Class
    ...
    _init: (data) ->
        @parent()
        @data = data
        @add new Gtk.Label(label: data)

ListBoxWindow = Lang.Class
    Name: "ListBoxWindow"
    Extends: Gtk.Window

    _init: () ->
        @parent title: "ListBox Demo"
        @border_width = 10
        ...
        items.forEach (item) => listbox2.add(new ListBoxRowWithData(item))

        sortFunc = (row1, row2, data, notifyDestroy) => row1.data.toLowerCase() > row2.data.toLowerCase()

        filterFunc = (row, data, notifyDestroy) => (row.data != 'Fail')
